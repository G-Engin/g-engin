#cs ----------------------------------------------------------------------------
 ...
 Remarque:
	- Il faut avoir lu le Tuto 1, car ce qui a �t� expliqu� ne le sera pas
...
; pour que le Sprite s'arr�te de bouger tout seul si il n'a pas d'acc�l�ration
; Plus le valeur est grande, plus l'arret du mouvement est rapide apr�s qu'il
; n'y ai plus d'acc�l�ration (voir la boucle principale)
...
Do
	...
	Select ; Mouvement
		Case _IsPressed('26') ; Haut
			_GEng_Sprite_AccelSet($spr_Gimp, Default, -500) ; On ne touche pas � l'acc�l�ration sur l'axe X (car $x = Default)
		Case _IsPressed('28') ; Bas
			_GEng_Sprite_AccelSet($spr_Gimp, Default, 500) ; Idem
		Case _IsPressed('25') ; Gauche
			_GEng_Sprite_AccelSet($spr_Gimp, -500, Default) ; On ne touche pas � l'acc�l�ration sur l'axe Y (car $y = Default)
		Case _IsPressed('27') ; Droite
			_GEng_Sprite_AccelSet($spr_Gimp, 500, Default) ; Idem
		Case Else
			_GEng_Sprite_AccelSet($spr_Gimp, 0, 0) ; acc�l�ration nul, c'est l'innertie qui va stoper le mouvement
	EndSelect
	; ---
	Select ; Rotation	
		...
		Case _IsPressed('10') ; Shift
			_GEng_Sprite_AngleAccelSet($spr_Gimp, -500)
		Case Else
			_GEng_Sprite_AngleAccelSet($spr_Gimp, 0) ; acc�l�ration nul, c'est l'innertie qui va stoper le mouvement
	EndSelect
	; ---
